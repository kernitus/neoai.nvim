...
The canonical API defaults live in `lua/neoai/config.lua`: `api.main` targets `gpt-5.1-codex-max` with 128,000 `max_output_tokens` and reasoning effort "medium". Keep documentation synchronised with that file whenever the presets change.
...
- On the first user turn of a new session, NeoAI always runs a bootstrap preflight that issues synthetic tool_calls for project context (SymbolIndex only) and immediately executes them locally before resuming the model. This behaviour is unconditional; you may customise the tool list via chat.bootstrap.tools but non-SymbolIndex entries are ignored and the preflight itself cannot be disabled.
...
- Configure at least the main API profile in setup: api.main. Defaults mirror `lua/neoai/config.lua`: `api.main` targets `gpt-5.1-codex-max` (128,000 `max_output_tokens`, reasoning effort "medium"). You may add other labelled profiles if you plan to support model switching. When configuring plugin managers, pair `build = "./gradlew shadowJar"` with an `init` job (e.g., `vim.fn.jobstart({ "bash", "-c", "cd " .. plugin.dir .. " && ./gradlew shadowJar" }, { detach = true })`) so the Kotlin jar is rebuilt on install/update and during Neovim start-up.
...
- Build Kotlin sidecar: `./gradlew shadowJar` (requires Java 17+). Run this before launching Neovim whenever `src/main/kotlin/**` changes so the latest jar is available to the plugin.
- Build (Lua): none required beyond Neovim.
